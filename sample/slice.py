...
class Slice(MCMCKernel):
    ...
    def _sample_from_conditional(self, params, dim):
        ...
        if params[''].get_device() < 0:
            device = 'cpu'
        else:
            device = 'cuda:'+str(params[''].get_device())

        # Sample uniformly from slice
        log_height = _log_prob_d(params[self._site_name].view(-1)[dim]).cpu().detach() + torch.log(
            torch.rand(1)
        )#.to(params[self._site_name].get_device())

        # Position the bracket randomly around the current sample
        lower = params[self._site_name].view(-1)[dim].cpu().detach() - self._width[dim] * torch.rand(1)#.to(params[self._site_name].get_device())
        upper = lower + self._width[dim]

        # Find lower bracket end
        while (
            _log_prob_d(lower.to(device)).cpu().detach() >= log_height
            and params[self._site_name].view(-1)[dim].cpu().detach() - lower < self._max_width
        ):
            lower -= self._width[dim]

        # Find upper bracket end
        while (
            _log_prob_d(upper.to(device)).cpu().detach() >= log_height
            and upper - params[self._site_name].view(-1)[dim].cpu().detach() < self._max_width
        ):
            upper += self._width[dim]

        # Sample uniformly from bracket
        new_parameter = (upper - lower) * torch.rand(1) + lower#.to(params[self._site_name].get_device()) + lower

        # If outside slice, reject sample and shrink bracket
        while _log_prob_d(new_parameter.to(device)).cpu().detach() < log_height:
            if new_parameter < params[self._site_name].view(-1)[dim].cpu().detach():
                lower = new_parameter
            else:
                upper = new_parameter
            new_parameter = (upper - lower) * torch.rand(1) + lower#.to(params[self._site_name].get_device()) + lower
