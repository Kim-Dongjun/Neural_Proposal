...
class Permutation(Transform):
    ...
    def _inverse_permutation(self):
        return torch.argsort(self._permutation).to(self._permutation.get_device())
    ...
    def _permute(inputs, permutation, dim):
        ...
        outputs = torch.index_select(inputs, dim, permutation).to(permutation.get_device())
        logabsdet = torch.zeros(batch_size).to(permutation.get_device())
