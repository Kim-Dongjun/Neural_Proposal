...
class StandardNormal(Distribution):
    ...
    def _log_prob(self, inputs, context):
        ...
        neg_energy = -0.5 * torchutils.sum_except_batch(inputs.to(self.device) ** 2, num_batch_dims=1)
        return neg_energy - self._log_z

    def _sample(self, num_samples, context):
        if context is None:
            return torch.randn(num_samples, *self._shape).to(self.device)
        else:
            # The value of the context is ignored, only its size is taken into account.
            context_size = context.shape[0]
            samples = torch.randn(context_size * num_samples, *self._shape).to(self.device)
            return torchutils.split_leading_dim(samples, [context_size, num_samples])

    def _mean(self, context):
        if context is None:
            return torch.zeros(self._shape).to(self.device)
        else:
            # The value of the context is ignored, only its size is taken into account.
            return torch.zeros(context.shape[0], *self._shape).to(self.device)


class ConditionalDiagonalNormal(Distribution):
    """A diagonal multivariate Normal whose parameters are functions of a context."""

    def __init__(self, shape, context_encoder=None, device='cpu'):
        """Constructor.
    ...
    def _sample(self, num_samples, context):
        ...
        # Generate samples.
        context_size = context.shape[0]
        noise = torch.randn(context_size * num_samples, *self._shape).to(self.device)
...
class DiagonalNormal(Distribution):
    ...
    def _log_prob(self, inputs, context):
        ...
        # Compute parameters.
        means = self.mean_.to(self.device)
        log_stds = self.log_std_.to(self.device)
        # assert means.shape == inputs.shape and log_stds.shape == inputs.shape

        # Compute log prob.
        norm_inputs = (inputs.to(self.device) - means) * torch.exp(-log_stds)
        log_prob = -0.5 * torchutils.sum_except_batch(
            norm_inputs ** 2, num_batch_dims=1
        )
        log_prob -= torchutils.sum_except_batch(log_stds, num_batch_dims=1)
