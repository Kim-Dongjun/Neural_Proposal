...
class UMNNMAF(nn.Module):
    ...
    def forward(self, x, method=None, x0=None, cond_inputs=None):
        x0 = x0.to(x.device) if x0 is not None else torch.zeros(x.shape).to(x.device)
        xT = x
        h = self.net.make_embeding(xT, cond_inputs)
    ...
    def compute_log_jac(self, x, cond_inputs=None):
        self.net.make_embeding(x, cond_inputs)
    ...
    def compute_log_jac_bis(self, x, cond_inputs=None):
        z = self.forward(x, cond_inputs=cond_inputs)
        jac = self.net.forward(x)
        return z, torch.log(jac + 1e-10) + self.scaling.unsqueeze(0).expand(x.shape[0], -1)

    def compute_ll(self, x, cond_inputs=None):
        z = self.forward(x, cond_inputs=cond_inputs)
        jac = self.net.forward(x)
    ...
    def compute_bpp(self, x, alpha=1e-6, cond_inputs=None):
        d = x.shape[1]
        ll, z = self.computeLL(x, cond_inputs=cond_inputs)
    ...
    def invert(self, z, iter=10, cond_inputs=None):
        nb_step = 10
        step = 1/(nb_step - 1)
        ...
        with torch.no_grad():
            for j in range(self.input_size):
                ...
                h = self.net.make_embeding(x_inv, cond_inputs)
...
class EmbeddingNetwork(nn.Module):
    ...
    def make_embeding(self, x_made, cond_inputs=None):
        #print("!!!! : ", x_made.shape, cond_inputs.shape)
        self.m_embeding = self.made.forward(x_made, cond_inputs)
