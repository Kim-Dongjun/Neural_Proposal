...
class AALR_with_ISP():
    def __init__(self, args):
        self.args = args
        self.dir = self.args.dir + '/'+str(self.args.simulation)+'/AALR_with_ISP' + '_' + self.args.likelihoodFlowType \
        ...
        self.netDiscriminator, self.optDiscriminator = networks.discriminatorNetwork(args)
        ...
        self.log_ = logs.logClass(args, self.dir, self.netDiscriminator, self.observation)
    ...
    def train(self, round):
        ...
        self.thetas = sample.sample(self.args, self.args.simulation_budget_per_round, self.netDiscriminator,
                                    self.netPosterior, self.observation, self.prior,
                                    self.sim, round == 0, self.args.posteriorInferenceMethod == 'no', self.args.numChains, SNLE=False).detach().to(self.args.device)
        ...
        if self.args.posteriorInferenceMethod != 'no':
            self.teacher_theta = sample.sample(self.args, self.args.num_training, self.netDiscriminator,
                                               self.netPosterior, self.observation, self.prior, self.sim,
                                               round == -1, True, self.args.num_training, parallel=True, SNLE=False)
    ...
    def plot(self, round):
        self.plot_.drawing(round, self.netDiscriminator, self.netPosterior, self.true_theta,
                                             self.training_theta, self.teacher_theta, self.thetas, self.sim, self.prior, SNLE=False)

    def log(self, round):
        self.log_.log(round, self.netDiscriminator, self.plot_.order, self.plot_, self.netPosterior)

if __name__ == '__main__':
    # Choose simulations among
    # complexPosterior : simulation with 4 modals with 5 dimensional input and 8 dimensional output
    # fourModes : simulation with 4 modals with 2 dimensional input and 20 dimensional output
    # multiModals : simulation with 16 modals with 2 dimensional input and 20 dimensional output
    # twoMoons : simulation with

    parser = argparse.ArgumentParser(description='')
    parser.add_argument("--simulation", default='multiModals', help="Simulation model to calibrate")
    parser.add_argument("--thetaDim", type=int, default=16, help="Simulation Input dimension")
    parser.add_argument("--xDim", type=int, default=10, help="Simulation output dimension")
    parser.add_argument("--numModes", type=int, default=9, help="Number of posterior modes")
    parser.add_argument("--numTime", type=int, default=200, help="Simulation execution timestep")
    ...
    parser.add_argument("--dir", default=os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))) + '/ThirdArticleExperimentalResults', help="Base directory")
    ...
    for i in range(10):
        aalr_with_isp = AALR_with_ISP(args)