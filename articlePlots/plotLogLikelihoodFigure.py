...
#base_dir = 'D:\Research\ThirdArticleExperimentalResults/200929_mg1/'
dirlist = os.listdir(base_dir)

filenames = ['Log_kde_posterior_performance']#, 'Inception_Score_performance']
...
for key in list(performance.keys()):
    ...
    perf = np.array(performance[key]) / 100
    #print(key, perf)
    mean = np.mean(perf, 0)
    std = np.std(perf, 0)
...
plt.tight_layout()
plt.show()
#plt.savefig('D:\Research\논문\Sequential_Likelihood_Free_Inference_with_Surrogate_Proposal\AISTATS21\imgs/complexPosterior16_LP_resize.pdf')
