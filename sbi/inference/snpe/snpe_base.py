...
class PosteriorEstimator(NeuralInference, ABC):
    ...
    def __call__(
        ...
        if calibration_kernel is None:
            calibration_kernel = lambda x: ones([len(x)]).to(device)
    ...
    def _train(
        ...
        dataset = data.TensorDataset(theta.to(device), x.to(device), prior_masks.to(device))
        ...
        optimizer = optim.Adam(
            list(self._posterior.net.parameters()), lr=learning_rate, betas=(0.5, 0.999), weight_decay=1e-5
        )
