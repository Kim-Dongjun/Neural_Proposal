...
class DirectPosterior(NeuralPosterior):
    ...
    def leakage_correction(
        ...
        default_x = self.default_x.to(device)

        # Check if the provided x matches the default x (short-circuit on identity).
        is_new_x = default_x is None or (
            x is not default_x and (x != default_x).any()
        )
        ...
        if is_new_x:  # Calculate at x; don't save.
            ...
            self._leakage_density_correction_factor = acceptance_at(default_x)
    ...
    def sample(
        ...
        if sample_with_mcmc:
            ...
            print("sample from rejection sampling")
            # Rejection sampling.
