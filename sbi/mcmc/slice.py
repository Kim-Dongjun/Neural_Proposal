...
class Slice(MCMCKernel):
    ...
    def _sample_from_conditional(self, params, dim):
        ...
        log_height = _log_prob_d(params[self._site_name].view(-1)[dim]).cpu().detach() + torch.log(torch.rand(1))

        # Position the bracket randomly around the current sample
        lower = params[self._site_name].view(-1)[dim].cpu().detach() - self._width[dim] * torch.rand(1)
